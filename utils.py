...
def Unormalize_mean(x):
    return (x - x.mean()) / (x.max() - x.min())

def to255(x):
    return (Unormalize(x) * 255.0).astype(np.uint8)
...
class Visdom():
    ...
    def Text(self, txt):
        self.vis.text('<h1>{}</h1>'.format(txt), win='Time')
    
    def Line(self, all_loss_mean, epoch):
        self.vis.line(win='Loss', Y=[all_loss_mean], X=[epoch], env=self.env, update='append')
        
    def MetricLine(self, metrics, epoch, name):
        for k in metrics.keys():
            self.vis.line(win='Metric', name="{}-{}".format(name, k), Y=[metrics[k]], X=[epoch], env=self.env, update='append')
        
    def Images(self, **kwargs):
        for key in kwargs:
            if 'Flow' in key:
                self.vis.images(kwargs[key], win=key, env=self.env, nrow=10, opts={'title': key})
            else:
                self.vis.images(generate_vimg(kwargs[key]), win=key, env=self.env, nrow=10, opts={'title': key})
...
def backwardwarping3D(x, flo, pad_mode='reflection'):
    ...
    grid = torch.cat((xx, yy, zz), 1).cuda() # [1, 3, 10, 160, 160]
...
#     # scale grid to [-1,1]
    ...
    output = F.grid_sample(x, vgrid, mode='bilinear', padding_mode=pad_mode, align_corners=True)
